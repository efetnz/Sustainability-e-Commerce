...
foreach ($cartRows as $cartRow) {
    ...
    if ($product) {
        // Check if quantity exceeds available stock and adjust if needed
        if ($cartRow['quantity'] > $product['stock']) {
            // Update quantity in database to match available stock
            $stmt = $conn->prepare("UPDATE cart SET quantity = ? WHERE id = ?");
            $stmt->execute([$product['stock'], $cartRow['cart_id']]);
            
            // Set notification message
            if (empty($error)) {
                $error = 'Some items in your cart were adjusted due to stock availability.';
            }
            
            // Update the quantity in our cart data
            $cartRow['quantity'] = $product['stock'];
        }
        
        // Combine cart and product info
        $cartItem = array_merge($cartRow, $product);
        $cartItem['subtotal'] = $cartItem['quantity'] * $cartItem['normal_price'];
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mb-3 cart-total">
                            ...
                            <span id="cart-total-final">
                                <?php
                                    $discountedTotal = $subtotal - $savings;
                                    echo number_format($discountedTotal, 2);
                                ?> TL
                            </span>
                        </div>
                        <div class="d-grid">
