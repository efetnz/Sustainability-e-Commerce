...
switch ($action) {
    case 'update':
        ...
        foreach ($cartItems as $item) {
            $cartTotal += $item['quantity'] * $item['normal_price'];
            $cartSavings += ($item['normal_price'] - $item['discounted_price']) * $item['quantity'];
        }
        $response = [
            'success' => true,
            'cart_total' => number_format($cartTotal - $cartSavings, 2),
            'cart_savings' => number_format($cartSavings, 2),
            'subtotal' => number_format($cartTotal, 2),
            'item_normal_price' => number_format($cartItem['normal_price'], 2),
        ];
    ...
    case 'purchase':
        ...
        $stmt = $conn->prepare("SELECT c.product_id, c.quantity, p.stock, p.discounted_price FROM cart c
            JOIN products p ON c.product_id = p.id WHERE c.consumer_id = ?");
        $stmt->execute([$profileId]);
        $cartItems = $stmt->fetchAll();
        ...
        foreach ($cartItems as $item) {
            ...
            $stmt = $conn->prepare("INSERT INTO purchases (consumer_id, product_id, quantity, purchase_price) 
                                  VALUES (?, ?, ?, ?)");
            $stmt->execute([
                $profileId, 
                $item['product_id'], 
            ...
            if ($product['stock'] == 0) {
                ...
                if (!empty($productImage['image_path'])) {
                    $imagePathToDelete = PRODUCT_IMAGE_UPLOAD_DIR . $productImage['image_path'];
