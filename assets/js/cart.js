    ...
    function updateCartQuantity(action, productId, cartId) {
        ...
        sendAjaxRequest(data, function(response) {
            if (response.success) {
                ...
                if (productSubtotal) {
                    // The server calculates the individual product subtotal based on normal price and quantity
                    const itemSubtotal = (parseFloat(response.item_normal_price) * currentQuantity).toFixed(2);
                    productSubtotal.textContent = itemSubtotal + ' TL';
                }
                
                // Update cart summary
                document.getElementById('cart-total').textContent = response.subtotal + ' TL';
